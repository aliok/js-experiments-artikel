...
(function (){ // START HIDING FROM GLOBAL SCOPE
    window.createJackContext = function(){
        var jackInstance = new Jack();
        jackInstance.matchers = new Matchers();
        jackInstance.util = new Util();
        jackInstance.FunctionSpecification = FunctionSpecification;
        jackInstance.FunctionGrab = FunctionGrab;
        ...
        function FunctionSpecification() {
            ...
            function mixinMatchers(api) {
                api.whereArgument = function(argIndex) {
                    var collected = {};
                    for(var name in jackInstance.matchers) {
                ...
                function addMatcher(argIndex, name, collection) {
                    collection[name] = function() {
                        addConstraint(argIndex, jackInstance.matchers[name], arguments);
                        if(name == "is") {
        ...
        function Matchers() {
            ...
            function result(match, actual, prefix, expected) {
                return {
                    result: match,
                    actual: jackInstance.util.displayValue(actual),
                    expected: jackInstance.util.displayValue(prefix, expected)
                }
            }
	}
    ...
    /** EXPORT JACK for singleton context use **/
    window.jack = createJackContext();
